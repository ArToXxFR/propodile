...
<div>
                    ...
                    @foreach ($users as $user)
                        <div class="flex flex-col sm:flex-row items-center justify-between border-b border-gray-200 p-4">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <button class="inline-flex items-center px-4 py-2 border bg-white border-red-500 text-red-500 rounded-md hover:bg-red-100 hover:text-red-500 focus:shadow-outline-green transition duration-300 ease-in-out" wire:click="confirmBanUser('{{ $user->id }}')">
                                    Bannir
                                </button>
                            </div>
                        </div>
    ...
    <x-confirmation-modal wire:model.live="confirmingBanUser">
        <x-slot name="title">
            {{ __('Bannir l\'utilisateur') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Êtes-vous sûr de vouloir bannir cette personne ?') }}
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmingBanUser')" wire:loading.attr="disabled">
                {{ __('Annuler') }}
            </x-secondary-button>

            <x-danger-button class="ms-3" wire:click="confirmBanUser" wire:loading.attr="disabled">
                {{ __('Bannir') }}
            </x-danger-button>
        </x-slot>
